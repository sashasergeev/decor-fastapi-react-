...
import WinFrameBox from "../Window/elements/WinFrameBox";
import Wall from "../elements/Wall";
import Door from "../elements/Door";
import DecorItem from "../elements/DecorItem";

const DoorCanvas = ({ size, decor }) => {
  ...
  const topDecor = decor.filter((e) => e.name === "Верхх")[0]?.chosen;
  const middleDecor = decor.filter((e) => e.name === "Серединаа")[0]?.chosen;
  const baseDecor = decor.filter((e) => e.name === "Базы")[0]?.chosen;
