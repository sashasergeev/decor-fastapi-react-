...
interface SkeletonElemProps {
  height: number;
  width: number;
}

export const SkeletElem = styled.div<SkeletonElemProps>`
...
export const SceneBox = styled.div<SceneBoxProps>`
  width: 100%;
...
interface SetttingBoxProps {
  $hide: boolean;
}
export const SettingBox = styled.div<SetttingBoxProps>`
  transition: 0.3s;
  overflow: hidden;
...
interface HideBoxBtnProps {
  body: string;
  bgHover: string;
  mediaqBody: string;
}
export const HideBoxBtn = styled.div<HideBoxBtnProps>`
  // common
  position: absolute;
  cursor: pointer;
  border: none;
...
export const Catalog = {
  ...
  CategoryItem: styled.div`
    padding: 5px 3px;
    background: ${({selected} : {selected: number}) => (selected ? "#673ab7" : "transparent")};
    border-radius: ${({selected} : {selected: number}) => (selected ? "5px" : "0px")};
...
export const ConstructorMenu = {
  Container: styled.div`
    height: ${({ fullSize } : {fullSize: number}) => (fullSize ? "80vh" : "")};
    width: 100vw;
    display: table-cell;
...
interface StyledPriceBoxProps {
  hide: number
}

export const StyledPrice = {
  Box: styled.div<StyledPriceBoxProps>`
